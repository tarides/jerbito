...
let division x y =
  match x, y with
  | Entier a, Entier b -> Entier (a / b)
  | Entier a, Flottant b | Flottant b, Entier a ->
  | Entier a, Flottant b | Flottant b, Entier a -> Flottant (float_of_int a /. b)
  | Flottant a, Flottant b -> Flottant (a /. b);;
  ...
  let ma_temperature = Celsius 25.0
  ...
  let score carte atout_opt =
  let score_normal = function
    | As -> 11 | Dix -> 10 | Roi -> 4 | Dame -> 3 | Valet -> 2 | _ -> 0
  and score_atout = function
    | As -> 11 | Dix -> 10 | Roi -> 4 | Dame -> 3 | Valet -> 20 | _ -> 0
  in
  match atout_opt with
  | None ->
      (match carte.valeur with
      | As -> 11 | Dix -> 10 | Roi -> 4 | Dame -> 3 | Valet -> 2 | _ -> 0)
  | Some atout ->
      if carte.couleur = atout then
        score_atout carte.valeur
      else
        score_normal carte.valeur
